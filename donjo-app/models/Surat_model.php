<?php class Surat_model extends CI_Model {
	...
	private function sisipkan_ttd($nama_logo, $buffer)
	{
		$file_logo = APPPATH . '../' . LOKASI_USER_TTD . $nama_logo;
		// var_dump($file_logo);die;
		if (!is_file($file_logo)) return $buffer;
		// Akhiran dan awalan agak panjang supaya unik
		$akhiran_logo = '801d36bc0000000049454e44ae426082';
		$awalan_logo = '89504e470d0a1a0a0000000d49484452000004380000043808060000019b';
		$akhiran_sementara = 'akhiran_logo';
		$jml_logo = substr_count($buffer, $akhiran_logo);
		if ($jml_logo <= 0) return $buffer; // tidak ada logo placeholder
	...
	*/
	...
	public function surat_rtf($data)
	{
		...
		if (is_file($file))
		{
			...
			$buffer = $this->sisipkan_ttd($input['ttd'], $buffer);
